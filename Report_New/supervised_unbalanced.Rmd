...
After tuning the hyperparameters for our neural network model (size of `r nnetFit$bestTune$size` and weight decay of `r nnetFit$bestTune$decay`), we get a pretty good model for predicting our outcomes. An accuracy of `r cm$overall['Accuracy']` and a moderate Cohen's Kappa of `r cm$overall['Kappa']`. Compared to the previous models, neural network is the best model to predict our outcomes for now.
...
We find that the best tune for C is equal to `r svm_Linear_Grid$bestTune$C`, and when we use this metric to fit the model, we obtain an accuracy of `r cm$overall['Accuracy']` and a pretty low Kappa: `r cm$overall['Kappa']`.
...
Now that we found that our best models are Neural Networks and Random Forest models, we need to see if we can improve their prediction quality, that we judge by accuracy and kappa, by getting rid of the variables that do not contribute to the prediction. to do this, we use model-specific variable importance methods.
...
```
#AMINA'S PART START
After applying these methods and plotting the "importance" of variables, we see that variables ..., ..., ... and ... are the ones who contribute most to the predictions of the game results. Indeed, not including these x variables can cause a decrease of the cross-entropy for x model, ... 

In the plot for Neural Networks above, for each feature, we the difference in entropy obtained by 10 permutations. It shows graphically the average variable importance according to the cross entropy loss.
...
*!* Idk how to interpret graph, so not sure if variables I took are the "right" ones, but I think it's the extreme one on top and bottom.

When we refit the Random Forest model with the 4 most "important" variables, accuracy and kappa decrease, from 0.713 to 0.639 and from 0.529 to 0.424 respectively. This is change can not be ignored, so it is probably better to keep the original model in this case.
...
VarImpNNfit <- train(full_time_results ~ half_time_away_goals + half_time_home_goals + away_shots_ontarget + home_shots_ontarget, 
                 data = train_data1,
                 method = "nnet",
                 metric = "Accuracy",
                 tuneGrid = VarImpGrid,
...
oldconfmatrixNN$overall
refitconfmatrixNN$overall
```


As for the Neural Network model with the 4 most "important" variables, accuracy and kappa decrease again, from 0.685 to 0.676 and from 0.457 to 0.45 respectively. This is change is very marginal though, so it still encourages us to take fewer variables for our modeling as simpler is often better !
