...
For our data split, we took 80% of the original data for our training set and the remaining 20% for our test set. As we have a large amount of data, we then went on further with a cross validation (CV) strategy on our training sets before fitting our models.
...
confusionMatrix(data=pred_rf, reference = test_data1$full_time_results)
...
For our random forest model, the hyperparameters are:
- Randomly selected predictors (mtry): 4
- Splitting rule: extratrees
- Minimal node size:7

After tuning our model, we got an accuracy of 0.694 and a Cohen's Kappa of 0.496.
...
Once we’ve identified influential variables across the 2 models, next we likely want to understand how the relationship between these influential variables and the average prediction response differ between the models.


#Refitting the model with the 4 most important variables
