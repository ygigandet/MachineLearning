...
## Data Splitting
Data splitting is when data is divided into two or more subsets. Typically, we take 80% for our training set and 20% for our test set. Splitting into Training and test set help to avoid overfitting and prepare our data to use machine learning models. To achieve our aim, the strategy which we use is Cross validation (CV) data splitting method. Since we have a large amount of data, the cross validation method seems the best one for our dataset.
...
Finally, We performed a random train and test split of our data in order to use our models.
...
y_validtraiforRF <- as.vector(as.numeric(y_train))

varImp(rf)
varImp(rf$finalModel)
explainer_RF <- DALEX::explain(model = rf$finalModel,
                                #is_multiclass = TRUE,
                                data = x_train,
                                y = y_validtraiforRF,
                                label = "Random Forestmodel")
...
importance_nn <- calculate_importance(explainer_RF)
