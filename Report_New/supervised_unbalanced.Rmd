...
We reach an accuracy of `r cm$overall['Accuracy']` and the Kappa of `r cm$overall['Kappa']`. 
...
We find that the best tune for C is equal to `r svm_tuned$bestTune$C`, and when we use this metric to fit the model, we obtain an accuracy of `r cm$overall['Accuracy']` and a pretty good Kappa: `r cm$overall['Kappa']`.
...
By having a look at the results, we see that Random Forest outperforms the other models by far, followed by Support Vector Machines and Neural Network. Therefore, we remove ? for the following parts, variable importance.
