...
table(train_data1_subs$full_time_results)
...
### ALESYA ###
index.home <- train_data1$full_time_results == "H"
index.draw <- train_data1$full_time_results == "D"
index.away <- train_data1$full_time_results == "A"

n.home <- sum(index.home)
n.draw <- sum(index.draw)
n.away <- sum(index.away)

set.seed(1234)
premier_league_results.rs <- rbind(train_data1[index.home], train_data1[sample(x=which(index.draw), size = n.home, replace=FALSE)], train_data1[sample(x=which(index.away), size = n.home, replace=FALSE)])
