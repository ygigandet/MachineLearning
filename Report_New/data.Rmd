...
As it can be seen, the variables concern the performance of either the home or the away team during a specific match. Before starting to analyze all these variables, we would like to provide a short explanation for each of them:

- full time goals: when we talk about full time, we refer to the 90 minutes the rule says a football game should last. In our case, full time goals represent the number of goals at the end of the match.
- half time goals: the number of goals scored in the first part of the game (first 45 minutes).
...
- shots: ball hits as an attempt to score a goal.
- shots on target: shots that enters the goal or would have entered the goal if they had not been blocked by the goalkeeper or another defensive player.
- fouls: inappropriate or unfair acts by a player as deemed by a referee, usually violating the rules of the sport or game (may be intentional or accidental, and often result in a penalty).
...
- yellow cards: given as a caution or warning, provide players with another chance to stay on the field for the remainder of the game.
- red cards: used by the officials to remove a player from the match with immediate effect. It means the automatic ejection of the player and that the player's team will remain shorthanded for the remainder of the match. Red cards can be given if the same player has received two yellow cards in the same match.
