...
#NNvarImp <- varImp(nnetFit, scale = FALSE)
#CARTvarImp <- varImp(classtreefit.pruned, scale =FALSE )

#install.packages("vi")
#library(vi)
# First, compute a tibble of variable importance scores using any method
#var_imp <- vi(nnetFit, method = "permute", metric = "auc", pred_wrapper = pfun,
              #target = "diabetes", reference_class = "neg")

# Next, convert to an html-based data table with sparklines
#add_sparklines(var_imp, fit = nnetFit)
```
...
#PAR CONTRE JE NE SUIS PAS SUR RF
rfeCtrl <- rfeControl(functions = rfFuncs,
                      method = "cv",
                      verbose = FALSE)

rfProfile <- rfe(x = x_train, 
                y = y_train, 
                #sizes = subsets,
                rfeControl = rfeCtrl)
rfProfile
