...
WinPercentage <- players_stats %>% mutate(Wins_Perc = Wins/Appearances, .after= Wins) 

highest_wins_perc <- plot_ly(WinPercentage, x = ~Name, y = ~Wins_Perc, type = 'bar', name = "Wins Percentage per player", width = 1000, height = 600 ) %>% 
...
highest_goals_defender <- plot_ly(goals_defender, x = ~Name, y = ~Goals_Perc, type = 'bar', name = ~Club, width = 1000, height = 600, color = ~Club ) %>%
  layout(title= 'Goals Percentage per Defender' )

highest_goals_defender
```
<br>
...
highest_goals_midfielder <- plot_ly(goals_midfielder, x = ~Name, y = ~Goals_Perc, type = 'bar', name = ~Club, width = 1000, height = 600 , color = ~Club) %>%
  layout(title= 'Goals Percentage per Midfielder' )

highest_goals_midfielder
```
<br>
...
highest_goals_forward <- plot_ly(goals_forward, x = ~Name, y = ~Goals_Perc, type = 'bar', name = ~Club, width = 1000, height = 600, color = ~Club ) %>%
  layout(title= 'Goals Percentage per Forward' )

highest_goals_forward
```
...
age_wins_corrplot <- plot(WinPercentage$Age, WinPercentage$Wins_Perc, main = "Age Vs Wins", pch = 19, col = "blue", xlab = "Age", ylab = "Percentage of Wins")
abline(lm(WinPercentage$Wins_Perc~WinPercentage$Age), col = "green", lwd = 3)
...
# corrplottest<- plot_ly(WinPercentage, x = ~Age, y = ~Wins_Perc , type = 'scatter', width = 1000, height = 600 ) 
