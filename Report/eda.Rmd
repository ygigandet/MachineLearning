...
win_percentage_plot <- plot_ly(win_percentage, x = ~Name, y = ~Wins_Perc, type = 'bar', name = "Wins Percentage per player", width = 1000, height = 600, color = ~Club ) %>%
...
<br> 
```{r, echo=FALSE}
#Goas to Shots percentage for each forward player
...
# Looking for plausible correlation within the "attack" category
p1 <- team_stats_EDA %>% 
  select(c(2, 4, 9:19)) %>% 
  GGally::ggpairs()
